		...
		<div id="sidebar" class="sidebar">
			...
			<div data-scrollbar="true" data-height="100%">
				...
				<ul class="nav">
					...
					<li class="active"><a href="shareholder-dashboard.php"><i class="fas fa-chart-bar"></i> <span>Dashboard</span></a></li>
					<li><a href="shareholder-inquiry.php"><i class="fas fa-money-bill-alt"></i> <span>Shareholder Inquiry</span></a></li>
    ...
    "series": [
        {
            ...
            "data": [
               <?php 
               $tablesql = "SELECT e.username, SUM(( B.prod_poamount - (D.prod_machinecost + D.prod_materialcost))-(ROUND(( B.prod_poamount - (D.prod_machinecost + D.prod_materialcost))*(A.stkh_percent/100),0))-(((B.prod_poamount - (D.prod_machinecost + D.prod_materialcost)) -  ROUND(( B.prod_poamount - (D.prod_machinecost + D.prod_materialcost))*(A.stkh_percent/100),0)) * .2)) as SHAREHOLDERAMT
                ...
                $tableresult = mysqli_query($connect, $tablesql) or die(mysqli_error($connect));
 				...
 				while ($row = mysqli_fetch_assoc($tableresult)) {
 					$year = $row['SHAREHOLDERAMT'];
                ?> 
 				{
                    "name": "<?php echo $year;?>",
                    "y": 
                    	 <?php 
			               $tablesql1 ="SELECT e.username, SUM(( B.prod_poamount - (D.prod_machinecost + D.prod_materialcost))-(ROUND(( B.prod_poamount - (D.prod_machinecost + D.prod_materialcost))*(A.stkh_percent/100),0))-(((B.prod_poamount - (D.prod_machinecost + D.prod_materialcost)) -  ROUND(( B.prod_poamount - (D.prod_machinecost + D.prod_materialcost))*(A.stkh_percent/100),0)) * .2)) as SHAREHOLDERAMT
			  					FROM jamsfms_r_stakeholder AS A
								INNER JOIN jamsfms_r_sales AS B
								ON A.fk_so_id = B.so_id
								INNER JOIN jamsfms_r_customer AS C
            ...
            ?>
             {
                ...
                "data": [
					<?php 
				     ...
				     ?>
		                        [
		                        "<?php echo $row['cname'].'-'.$row['SHAREHOLDERAMTS']?>",
		                        <?php echo $row['SHAREHOLDERAMTS']?>
		                        ],
                        <?php } ?>
                ]
            },
